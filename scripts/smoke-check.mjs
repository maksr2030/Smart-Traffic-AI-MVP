...
const requiredFiles = [
  ...
  'engine/riskAwareRoutingEngine.js','engine/preventiveCommandEngine.js','engine/dynamicRiskTwinEngine.js','engine/predictiveOrchestrationEngine.js','coverage/coverageModel.js',
...
if (!html.includes('Dynamic Risk Digital Twin')) throw new Error('Dynamic Risk Digital Twin identity missing');
if (!app.includes('MVP v1.7') || !app.includes('Predictive Risk & Autonomous Orchestration')) throw new Error('runtime MVP v1.7 predictive orchestration identity missing');
...
console.log(`Static smoke check passed: ${new Set(requiredFiles).size} files, ${requiredIds.length} static DOM bindings + ${predictiveIds.length} runtime predictive bindings, ${network.nodes.length} nodes, ${network.edges.length} edges, ${scenarios.length} scenarios, ${fleet.length} fleet units, ${qcsObservations.length} QCS observations, Dynamic Risk Twin + Predictive Orchestration wired.`);
